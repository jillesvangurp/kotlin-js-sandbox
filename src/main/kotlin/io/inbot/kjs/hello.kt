...
fun main(vararg args: String) {
    ...
    var counter = 1
    setChildren(
        body, para(text("Some silly things driving redom using kotlin-js")),
        unorderedList(
            listItem(el("b", "foo")),
            listItem(el("a", json("href" to "http://inbot.io", "onClick" to { println("hi") }), "I can has links too")),
            listItem(el("a",
                json("onclick" to {
                    moreStuff.textContent =
                            "I can has kotlin lambdas as callbacks on links. You clicked me ${counter++} times"
                }),
                "Click me"
            )
            )
